...
int JournalTool::main_event(std::vector<const char*> &argv)
{
  ...
  } else if (command == "recovery_dentries") {
    r = js.scan();
    if (r) {
      derr << "Failed to scan journal (" << cpp_strerror(r) << ")" << dendl;
      return r;
...
int JournalTool::scavenge_dentries(EMetaBlob const &metablob, bool const dry_run)
{
       ...
       lp != metablob.lump_order.end(); ++lp)
  {
    ...
    if (r != 0) {
      derr << __func__ << ": failed to read OMAP header from directory fragment "
    ...
    if (old_fnode.version < lump.fnode.version && !dry_run) {
      bufferlist fnode_bl;
    ...
    for (list<ceph::shared_ptr<EMetaBlob::fullbit> >::const_iterator fbi = fb_list.begin(); fbi != fb_list.end(); ++fbi) {
      ...
      dout(4) << __func__ << ": inspecting frag " << frag_oid.name << " dentry " << fb.dn << dendl;
